const wSet= WeakSet || Set;
/** @type {WeakMap<fpubsubTopic, { listeners?: Set<function>, listeners_once: WeakSet<function>, value?: any }>} */
...
export function topic(options){
	...
	const storage_data= { listeners: new Set(), listeners_once: new wSet() };
...
export function isTopic(candidate){
...
export function subscribe(topic, listener, { once= false, signal }= {}){
	...
	if(once) storage.get(topic).listeners_once.add(listener);
	return 0;
}
...
export function unsubscribe(topic, listener){
	if(isInactiveTopic(topic)) return 1;
	const s= storage.get(topic);
	if(s.listeners_once.has(listener))
		s.listeners_once.delete(listener);
	return s.listeners.delete(listener) ? 0 : 2;
}
export const unsub= unsubscribe;
export function unsubscribeAll(topic){
	if(isInactiveTopic(topic)) return 1;
	Object.assign(storage.get(topic),
		{ listeners: new Set(), listeners_once: new wSet() });
	return 0;
}
